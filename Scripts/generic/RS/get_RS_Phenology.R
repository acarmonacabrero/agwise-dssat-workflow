...
Phenology_rasterTS<-function(country, useCaseName, crop, level, admin_unit_name, Planting_year, Harvesting_year, Planting_month, Harvesting_month, emergence, CropMask=T, CropType=F, coord, thr = c(0.10,0.10), validation = T, overwrite = FALSE){
  ...
  #' @return raster files of planting date, harvesting date and length of the cropping season at the Use Case level, the results will be written out in /agwise-planting-date-and-cultivar/Data/useCase/crop/result/RS/RSPlantingDate/
  #'
  ...
  if (validation == TRUE) {
    pathInG <- paste("/home/jovyan/agwise-planting-date-and-cultivar/Data/useCase_", country, "_",useCaseName, "/", crop, "/raw/RS/", sep="")
  ...
  if (CropMask == FALSE){
    ...
    if (CropMask == TRUE & CropType == FALSE){
      ...
      if (CropMask == TRUE & CropType == TRUE){
        ...
        Crop_pathIn <- paste0("/home/jovyan/agwise-planting-date-and-cultivar/Data/useCase_", country, "_",useCaseName, "/", crop, "/result/RS/CropType")
