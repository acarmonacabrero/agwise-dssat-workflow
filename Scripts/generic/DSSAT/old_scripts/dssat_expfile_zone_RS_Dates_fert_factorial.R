...
dssat.expfile <- function(country, useCaseName, Crop, project_root, AOI = TRUE,
  ...
  if(AOI == TRUE) {
    ...
    if (Forecast) {
      countryCoord <- readRDS(
        paste0(project_root, "/Data/data_curation/",
               country, "/AOI_GPS.RDS"))
    } else if (!Forecast) {
  ...
  if(AOI == TRUE) {
    path.to.extdata <- paste(project_root, "/Data/useCase_", country, "_",useCaseName, "/", Crop, "/transform/DSSAT/AOI/",varietyid, sep="")
  }else{
    path.to.extdata <- paste(project_root, "/Data/useCase_", country, "_",useCaseName, "/", Crop, "/transform/DSSAT/fieldData/",varietyid, sep="")
  }
  if (!dir.exists(path.to.extdata)) dir.create(path.to.extdata, recursive = TRUE)
  
  path.to.temdata <- paste(project_root, "/Data/useCase_", country, "_",useCaseName, "/", Crop, "/Landing/DSSAT", sep="")
