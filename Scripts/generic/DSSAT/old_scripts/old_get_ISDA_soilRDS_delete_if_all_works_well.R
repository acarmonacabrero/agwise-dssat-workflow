...
# TODO: Is this used? Maybe remove it
## Produce ISDA RDS file
get_ISDA_soilRDS <- function(
  ...
  for (prov in unique(inputData$NAME_1)) {
    ...
    pathIn <- define_pathIn(general_pathIn, level2 = NA, zone = prov, pathIn_zone = T, Forecast = F)
    saveRDS(pointDataSoil, paste0(pathIn, "/ISDA_SoilDEM_PointData_AOI_profile.RDS"))
  }
}
